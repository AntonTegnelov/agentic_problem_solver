...
@pytest.fixture
def mock_provider():
    return MockLLMProvider("test_key")
...
@pytest.mark.asyncio
async def test_factory_provider_switching(mock_env, reset_factory):
    """Test switching providers in LLMProviderFactory."""
    factory = LLMProviderFactory()
    factory.register_provider("mock", MockLLMProvider)
    original_provider = factory.get_provider()

    # Switch to mock provider
    factory.set_provider("mock", api_key="new_key")
    new_provider = factory.get_provider()

    assert isinstance(original_provider, GeminiProvider)
    assert isinstance(new_provider, MockLLMProvider)
    assert new_provider.api_key == "new_key"
