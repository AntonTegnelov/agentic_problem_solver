...
def test_validate_task_invalid_dependency_task_id() -> None:
    ...
    task = Task(description="Test task")
    dependency = TaskDependency(task_id="not-a-uuid", description="Dependency")  # type: ignore[arg-type]
...
def test_validate_task_list_invalid_item() -> None:
    ...
    tasks = [Task(description="Task 1"), "not-a-task"]  # type: ignore[list-item]
    is_valid, error = validate_task_list(tasks)
    assert not is_valid
